...
yearDropdownOptions = np.append([allYears], crime_df[applicationconstants.AÑO].unique())
                                    ...
                                    dbc.Row(
                                        [
                                            ...
                                            dbc.Col([
                                                ...
                                                dcc.Dropdown(
                                                    id="personalinjury-diasemana",
                                                    placeholder=applicationconstants.dropdown_placeholder,
                                                    options=[
                                                        {"label": col, "value": col} for col in crime_df[applicationconstants.DIA_SEMANA].str.capitalize().unique()
                                                    ],
                                                ),
                                            ], width="3"),
                                            dbc.Col([
                                                ...
                                                dcc.Dropdown(
                                                    id="personalinjury-barrio",
                                                    placeholder=applicationconstants.dropdown_placeholder,
                                                    options=[
                                                        {"label": col, "value": col} for col in crime_df[applicationconstants.UNIDAD_ESPACIAL].str.title().unique()
                                                    ],
                                                ),
                                            ], width="3"),
                                            dbc.Col([
                                                ...
                                                dcc.Dropdown(
                                                    id="personalinjury-grupoetario",
                                                    placeholder=applicationconstants.dropdown_placeholder,
                                                    options=[
                                                        {"label": col, "value": col} for col in crime_df[applicationconstants.GRUPO_ETARIO_VICTIMA].str.capitalize().unique()
                                                    ]
                                                ),
                                            ], width="3")
                                        ], style={"padding": "0 16px 0 16px"}),
                                    dcc.Graph(id="personalinjury-graph"),
                                    html.Div(
                                        dbc.Checklist(
                                            id="personalinjury-diasemana-toggle",
                                            value=[0],
                                            switch=True,
                                            options=[
...
@app.callback(
    Output("personalinjury-graph", "figure"),
    [
     Input("personalinjury-year", "value"),
     Input("personalinjury-diasemana", "value"),
     Input("personalinjury-barrio", "value"),
     Input("personalinjury-grupoetario", "value"),
    Input("personalinjury-diasemana-toggle", "value")
    ]
)
def generate_graphic(year, week_day, barrio, grupo_etario, show_by_week_day):
    cases_df = crime_df[crime_df[applicationconstants.TIPO_DELITO] == "LESIONES PERSONALES"]
    if not year:
        year = 2010
    ...
    if len(show_by_week_day) == 1:
        injury_type_df = cases_df.groupby([applicationconstants.TIPO_CONDUCTA]).size().reset_index(name="Casos")
        injury_type_df = injury_type_df.sort_values(by="Casos", ascending=False)
        fig = px.bar(
            injury_type_df,
            x=applicationconstants.TIPO_CONDUCTA,
            y="Casos",
