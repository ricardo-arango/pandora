...
modal_instance = dbc.Modal(
    [
        dbc.ModalHeader(
            html.H5("Homicidios", style={"font-family": "revert", "color": "#5f5f5f"})
        ),
        dbc.ModalBody(
            dbc.Row(
                [
                        ...
                        [
                            html.Div(
                                [
                                    ...
                                    dcc.Graph(id="homicide-graph"),
                                    html.Div(
                                        dbc.Checklist(
                                            id="homicide-diasemana-toggle",
                                            value=[0],
                                            switch=True,
                                            options=[
        ...
        dbc.ModalFooter(
            dbc.Button(
                "Cerrar", id="homicide-close", className="ml-auto", n_clicks=0
            )
        ),
    ],
    id="homicide-modal",
...
@app.callback(
    Output("homicide-graph", "figure"),
    [
     Input("homicide-year", "value"),
     Input("homicide-diasemana", "value"),
     Input("homicide-barrio", "value"),
     Input("homicide-grupoetario", "value"),
     Input("homicide-diasemana-toggle", "value")
    ]
)
def generate_graphic(year, week_day, barrio, grupo_etario, show_by_week_day):
    cases_df = dataloading.crime_df[dataloading.crime_df[applicationconstants.TIPO_DELITO] == "HOMICIDIO"]
    if not year:
        year = 2010
    ...
    if len(show_by_week_day) == 1:
        ...
        fig = px.bar(
            ...
            labels={applicationconstants.TIPO_CONDUCTA: applicationconstants.behaviour_type_label, applicationconstants.TIPO_DELITO: "Tipo Delito"},
            height=800
        )
    else:
