...
modal_instance = dbc.Modal(
    [
        dbc.ModalHeader(
            html.H5("Hurto a personas", style={"font-family": "revert", "color": "#5f5f5f"})
        ),
        dbc.ModalBody(
            dbc.Row(
                [
                        ...
                        [
                            html.Div(
                                [
                                    ...
                                    dcc.Graph(id="theftpeople-graph"),
                                    html.Div(
                                        dbc.Checklist(
                                            id="theftpeople-diasemana-toggle",
                                            value=[0],
                                            switch=True,
                                            options=[
        ...
        dbc.ModalFooter(
            dbc.Button(
                "Cerrar", id="theftpeople-close", className="ml-auto", n_clicks=0
            )
        ),
    ],
    id="theftpeople-modal",
    size="xl",
    is_open=False
)
...
@app.callback(
    Output("theftpeople-graph", "figure"),
    [
     Input("theftpeople-year", "value"),
     Input("theftpeople-diasemana", "value"),
     Input("theftpeople-barrio", "value"),
     Input("theftpeople-grupoetario", "value"),
     Input("theftpeople-diasemana-toggle", "value")
    ]
)
def generate_graphic(year, week_day, barrio, grupo_etario, show_by_week_day):
    cases_df = dataloading.crime_df[dataloading.crime_df[applicationconstants.TIPO_DELITO] == "HURTO A PERSONAS"]
    if not year:
        year = 2010
    ...
    if len(show_by_week_day) == 1:
        injury_type_df = cases_df.groupby([applicationconstants.TIPO_DELITO, applicationconstants.AÑO, applicationconstants.TIPO_CONDUCTA]).size().reset_index(name="Casos")
        ...
        fig = px.bar(
            ...
            labels={applicationconstants.TIPO_CONDUCTA: applicationconstants.behaviour_type_label, applicationconstants.AÑO: "Año", applicationconstants.TIPO_DELITO: "Tipo Delito"},
            height=450
        )
    else:
