...
def generate_graphic(year, week_day, barrio, grupo_etario, show_by_week_day):
    cases_df = dataloading.crime_df[dataloading.crime_df["TIPO_DELITO"] == "HURTO A PERSONAS"]
    if not year:
        year = 2010
    ...
    if len(show_by_week_day) == 1:
        injury_type_df = cases_df.groupby(["TIPO_DELITO", "AÑO", "TIPO_CONDUCTA"]).size().reset_index(name="Casos")
        ...
        fig = px.bar(
            ...
            labels={"TIPO_CONDUCTA": applicationconstants.behaviour_type_label, "AÑO": "Año", "TIPO_DELITO": "Tipo Delito"},
            height=450
        )
    else:
