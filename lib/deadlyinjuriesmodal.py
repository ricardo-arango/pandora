...
modal_instance = dbc.Modal(
    [
        dbc.ModalHeader(
            html.H5("Lesiones fatales", style={"font-family": "revert", "color": "#5f5f5f"})
        ),
        dbc.ModalBody(
            dbc.Row(
                [
        ...
        dbc.ModalFooter(
            dbc.Button(
                "Cerrar", id="deadlyinjuries-close", className="ml-auto", n_clicks=0
            )
        ),
    ],
    id="deadlyinjuries-modal",
    size="xl",
    is_open=False
)
...
def generate_graphic(year, week_day, barrio, grupo_etario, show_by_week_day):
    cases_df = dataloading.crime_df[dataloading.crime_df[applicationconstants.TIPO_LESION] == "LESIONES FATALES"]
    if not year:
        year = 2010
